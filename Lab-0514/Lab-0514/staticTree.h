...
public:
	...
	long inOrderArray(long node, type* result, long index = 0) {
		if (array[node] != NULL) {
			index = inOrderArray(getLeft(node), result, index);
			result[index++] = array[node];
			index = inOrderArray(getRight(node), result, index);
		}
		return index;
	}
	...
	void mergeTree(staticTree<type>* tree) {
		...
		long summaryLength = numberOfElements + tree->numberOfElements;
		inOrderArray(0, first);
		tree->inOrderArray(0, second);
		...
		staticTree<type>* result = new staticTree(length + tree->length);
