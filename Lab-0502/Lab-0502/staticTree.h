...
public:
	...
	long numberOfSame(long node, type element) {
		long numberOf = 0;
		while (array[node] != NULL) {
			if (array[node] == element)
				numberOf++;
	...
	long differentNodes(long node, long different) {
		if (array[node] == NULL)
			return different;
		
		long current = numberOfSame(node, array[node]);
		if (current > 1)
			different--;

		different = differentNodes(getLeft(node), different);
		different = differentNodes(getRight(node), different);

		return different;
	}

	long nodesTotal(long node) {
		if (array[node] == NULL)
			return 0;

		long current = 1;
		current += nodesTotal(getLeft(node));
		current += nodesTotal(getRight(node));
